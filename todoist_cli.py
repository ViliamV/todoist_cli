...
class View:
    ...
    def set_parent(self, parent):
        """Seting the parent View of an instance"""
        self.parent = parent
    ...
    def print(self, items):
        ...
        with term.location(0, 0):
            ...
            for i, item in enumerate(items):
                print('> {}'.format(item)) if i == self.selected else print('  {}'.format(item))
    ...
    def update(self):
        ...
        elif isinstance(self.update_args, tuple):
...
class TaskList(View):
    def print(self):
        ...
        super().print(['{}'.format(t.content) for t in self.items])
    def update(self):
        super().update()
    ...
    def read_input(self):
        self.print()
        super().read_input(self.complete_task, self.undo_completion, super().pass_function,
                           super().go_back)
...
class Menu(View):
    ...
    def read_input(self):
        self.print()
        super().read_input(super().go_to, super().pass_function, super().go_to,
                           super().pass_function)
...
class Projects(View):
    ...
    def update(self):
        ...
        self.items = [TaskList(p.name, p.name, None, update_func=self.projects[i].get_tasks)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CACHE):
        os.makedirs(CACHE)
    try:
        keys = pickle.load(open(DIR+'/keys', 'rb'))
        login, password = pickle.load(open(DIR+'/login', 'rb'))
    except:
        print('Missing \'keys\' or \'login\'')
    user = todoist.login(login, password)
    today = TaskList('today', 'Today', '', update_args=(todoist.Query.TODAY, todoist.Query.OVERDUE),
                     update_func=user.search_tasks)
    tomorrow = TaskList('tomorrow', 'Tomorrow', update_args=todoist.Query.TOMORROW,
                        update_func=user.search_tasks)
